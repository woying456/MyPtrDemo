...
import com.android.gmacs.downloader.image.ImageLoader.ImageContainer;
import com.android.gmacs.downloader.image.ImageLoader.MultiImageContainer;
import com.android.gmacs.downloader.image.ImageRequest;
import com.common.gmacs.utils.GmacsEnvi;

import static com.android.gmacs.downloader.image.ImageRequest.DRAW_SHAPE_CIRCLE;
...
public class NetworkImageView extends AppCompatImageView {
    ...
    private MultiImageContainer mMultiImageContainer;
    ...
    void loadImageIfNecessary(final boolean isInLayoutPass) {
        ...
        if (TextUtils.isEmpty(mUrl) && mUrls == null) {
            if (mImageContainer != null) {
                mImageContainer.cancelRequest();
                mImageContainer = null;
            }
            if (mMultiImageContainer != null) {
                mMultiImageContainer.cancelRequest();
            ...
            if (mMultiImageContainer != null) {
                String[] fetchingUrls = mMultiImageContainer.getRequestUrls();
                if (fetchingUrls != null) {
                    if (fetchingUrls.length == mUrls.length) {
                        for (int i = 0; i < mUrls.length; i++) {
                            if (!TextUtils.equals(fetchingUrls[i], mUrls[i])) {
                                break;
                            } else if (i == mUrls.length - 1) {
                                return;
                            }
                        }
                    }
            ...
            mMultiImageContainer = RequestManager.getInstance().getImageLoader().get(mUrls, new ImageLoader.MultiImageListener() {
                @Override
                public void onResponse(final MultiImageContainer response, boolean isImmediate) {
            ...
            }, new MultiImageComposer(maxWidth, maxHeight), maxWidth, maxHeight, scaleType, ImageRequest.DRAW_SHAPE_RECT, 0);
        } else if (mUrls == null) {
            if (mMultiImageContainer != null) {
                mMultiImageContainer.cancelRequest();
                mMultiImageContainer = null;
