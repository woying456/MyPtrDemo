...
public class SoundRecord {
    ...
    private RecordSoundDialog recordSoundDialog;
    ...
    public boolean startRecord(Context context, boolean isShowDialog, RecordListener listener) {
        ...
        AudioManager audioManager = (AudioManager) GmacsEnvi.appContext.getSystemService(Context.AUDIO_SERVICE);
        ...
        try {
            ...
            recorder.start();
            if (isShowDialog) {
                recordSoundDialog = new RecordSoundDialog(context, R.style.publish_btn_dialog);
            ...
            // GN151. java.lang.RuntimeException : setAudioSource failed.
    ...
    private static class AnimHandler extends Handler {
        ...
        private WeakReference<SoundRecord> soundRecordWeakReference;
        private WeakReference<RecordSoundDialog> recordSoundAnimationWeakReference;

        AnimHandler(SoundRecord soundRecord, RecordSoundDialog recordSoundDialog) {
            soundRecordWeakReference = new WeakReference<>(soundRecord);
            recordSoundAnimationWeakReference = new WeakReference<>(recordSoundDialog);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            // 只有到9s的时候才有提示
            SoundRecord soundRecord = soundRecordWeakReference.get();
            if (soundRecord == null) {
                return;
            }
            RecordSoundDialog recordSoundDialog = recordSoundAnimationWeakReference.get();
            if (recordSoundDialog == null) {
                return;
            }

            if (soundRecord.recordTime >= 1 && soundRecord.recordTime <= 9) {
                recordSoundDialog.recordCountdownView.setVisibility(View.VISIBLE);
                recordSoundDialog.recordCountdownView.setText(String.valueOf(soundRecord.recordTime));
                recordSoundDialog.recording.setVisibility(View.GONE);
                recordSoundDialog.failedRecordView.setVisibility(View.GONE);
                if (recordSoundDialog.anim != null && recordSoundDialog.anim.isRunning()) {
                    recordSoundDialog.anim.stop();
                }
            }
        }
    }
    ...
    private class RecordSoundDialog extends AlertDialog implements OnShowListener {

        private TextView recordHint;
        private ImageView recording;

        private AnimHandler handler = new AnimHandler(SoundRecord.this, this);
        ...
        RecordSoundDialog(Context context, int theme) {
            super(context, theme);
